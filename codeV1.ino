...
<html>
  ...
  <body class="noselect" align="center" style="background-color:white">
    ...
    <h1 style="color: teal;text-align:center;">Phoenix ECHO</h1>
    <h2 style="color: teal;text-align:center;">Disaster response project</h2>
...
void sendCurrentRobotArmState()
{
  ...
  {
    wsCarInput.textAll(servoPins[i].servoName + "," + servoPins[i].servo.read());
  }
  wsCarInput.textAll(String("Record,") + (recordSteps ? "ON" : "OFF"));
  wsCarInput.textAll(String("Play,") + (playRecordedSteps ? "ON" : "OFF"));  
}
...
void playRecordedRobotArmSteps()
{
  ...
  for (int i = 0; i < 4 && playRecordedSteps; i++)
  {
    ...
    while (currentServoPosition != recordedStep.value && playRecordedSteps)  
    {
      ...
      wsCarInput.textAll(servoPins[recordedStep.servoIndex].servoName + "," + currentServoPosition);
      delay(50);
    }
  }
  ...
  for (int i = 4; i < recordedSteps.size() && playRecordedSteps ; i++)
  {
    ...
    wsCarInput.textAll(servoPins[recordedStep.servoIndex].servoName + "," + recordedStep.value);
  }
}
